    ...
    public class OrdersRepository : RepositoryBase<Order>, IOrderRepository
    {
        ...
        public async Task<PriceReportModel> GetPriceOrdersReports(DateFilterRequest request, MonthFilterRequest monthFilter)
        {
            ...
            if (request.DateFilter != "")
            {
                ...
                report.TotalProfitOrder = (double)lstOrder.Where(p => p.Status == (int)OrderStatusEnum.Completed).Sum(o => o.ShipCost * 0.2) + (double)lstPayment.Where(p => p.Type == (int)PaymentEnum.VNPay).Sum(o => o.Amount * 0.2) + (double)lstPayment.Where(p => p.Type == (int)PaymentEnum.Cash).Sum(o => o.Amount * 0.2); // Lợi nhuận // Lợi nhuận
        ...
        public async Task CreateTransaction(string shipperId, string orderId, int actionType)
        {
            ...
            else
            {
                ...
                var debitWallet = await context.Wallets.Where(x => x.AccountId == shipperId && x.Type == (int)WalletTypeEnum.Debit).FirstOrDefaultAsync();
